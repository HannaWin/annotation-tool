...
def annotate(index, text, annotation):
	"""work flow for the annotation """
	...
	for s in sentences[index:]:
		...
		i_w = 0
		if label == "exit":
			save_progress(annotation, index)
			break
		elif label == "sentence":
			print(s)
...
def save_progress(annotation, index):
	"""saves progress with annotation and index"""
...
if __name__ == '__main__':
	...
	annotation = fetch_annotation(index)

	annotate(index, text, annotation)
